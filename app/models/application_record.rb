class ApplicationRecord < ActiveRecord::Base
  ...
  def self.search_colegues(search, org_id)
    if search != ''
      where('name ILIKE ? OR surename ILIKE ? OR patronymic ILIKE ? OR post ILIKE ?  OR email ILIKE ? OR primary_phone_number ILIKE ? OR unitphoto ILIKE ? OR characteristic ILIKE ?', '%' + search + '%', '%' + search + '%', '%' + search + '%', '%' + search + '%', '%' + search + '%', '%' + search + '%', '%' + search + '%', '%' + search + '%').rewhere(organization_id: org_id.to_s)
    end
  end
