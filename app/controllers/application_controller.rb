class ApplicationController < ActionController::Base
  ...
  def all_organizations_departaments
    @all_organizations_departaments ||= Departament.where(organization_id: params[:organization_id])
  end
  helper_method :all_organizations_departaments
  ...
  def configure_permitted_parameters
    attributes =
      %i[name surename patronymic belong_to_departament post
         email secondary_email password password_confirmation
         primary_phone_number secondary_phone_number short_phone_nunber
         fax home_phone_number web_page start_work finish_work working
         days birthday unitphoto characteristic role subordinate]
