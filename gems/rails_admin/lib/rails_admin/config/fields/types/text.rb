    ...
    module Fields
      module Types
        class Text < RailsAdmin::Config::Fields::Base
          ...
          %i[ckeditor ckeditor_base_location ckeditor_config_js ckeditor_location].each do |key|
          ...
          %i[codemirror codemirror_assets codemirror_config codemirror_css_location codemirror_js_location].each do |key|
            register_deprecated_instance_option key do
              raise("The 'field(:foo){ codemirror true }' style DSL is deprecated. Please use 'field :foo, :code_mirror' instead.")
            end
          end

          %i[bootstrap_wysihtml5 bootstrap_wysihtml5_config_options bootstrap_wysihtml5_css_location bootstrap_wysihtml5_js_location].each do |key|
            register_deprecated_instance_option key do
